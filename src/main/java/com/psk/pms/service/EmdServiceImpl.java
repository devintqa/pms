...
public class EmdServiceImpl implements EmdService {
	...
	public boolean createEditEmd(EmdDetail emdDetail) {
		...
		emdDetail.setSqlEmdEndDate(getSQLDate(emdDetail.getEmdEndDate(),
				formatter));
		emdDetail.setSqlEmdStartDate(getSQLDate(emdDetail.getEmdStartDate(),
				formatter));
		emdDetail.setEmdExtensionSqlDate(getSQLDate(
				emdDetail.getEmdExtensionDate(), formatter));
	...
	public List<EmdDetail> getEmdDetails() {
		...
		for (EmdDetail emdDetail : emdDetails) {
			emdDetail.setEmdStartDate(DateFormatter.getStringDate(
					emdDetail.getSqlEmdStartDate(), formatter));
			emdDetail.setEmdEndDate(DateFormatter.getStringDate(
					emdDetail.getSqlEmdEndDate(), formatter));
	...
	public EmdDetail getEmdDetailsByEmdId(String emdId) {
		...
		emdDetail.setEmdEndDate(DateFormatter.getStringDate(
				emdDetail.getSqlEmdEndDate(), formatter));
		emdDetail.setEmdExtensionDate(DateFormatter.getStringDate(
				emdDetail.getEmdExtensionSqlDate(), formatter));
		return emdDetail;
	}
	...
	public List<EmdDetail> getEmdDetailsBySubProjectId(Integer subProjectId) {
		List<EmdDetail> emdDetails = emdDAO
				.getEmdDetailsBySubProjectId(subProjectId);
		SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
		for (EmdDetail emdDetail : emdDetails) {
			emdDetail.setEmdStartDate(DateFormatter.getStringDate(
					emdDetail.getSqlEmdStartDate(), formatter));
	...
	public List<EmdDetail> getEmdEndAlertList() {
		...
		for (EmdDetail emdDetail : projectDocumentList) {
			long diff = emdDetail.getSqlEmdEndDate().getTime()
					- todayDate.getTime();
