...
public class SignUpValidator extends BaseValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (!(employee.getEmployeePwd()
			...
			errors.rejectValue("employeePwd", "employeePwd.incorrect",
		...
		if (!(employee.getEmployeeMobile() != null && employee
			...
			if (!matcher.matches()) {
				...
				boolean isEmployeeMobNumExisting = employeeService
						.isEmployeeMobNumExisting(employee.getEmployeeMobile());
				if (isEmployeeMobNumExisting) {
					errors.rejectValue("employeeMobile",
							"employeeMobile.incorrect",
							"Mobile Number Already Found To Be Existing.");
				}
			}
		}
		if (!(employee.getEmployeeMail() != null && employee.getEmployeeMail()
			...
			if (!matcher.matches()) {
				...
				boolean isEmployeeMailExisting = employeeService
						.isEmployeeMailExisting(employee.getEmployeeMail());
				if (isEmployeeMailExisting) {
					errors.rejectValue("employeeMail",
							"employeeMail.incorrect",
							"Mail Id Already Found To Be Existing.");
				}
			}
		}

		if (employee.getEmployeeFName() != null
				&& !employee.getEmployeeFName().isEmpty()) {
		...
		if (employee.getEmployeeLName() != null
				&& !employee.getEmployeeLName().isEmpty()) {
			pattern = Pattern.compile(NAME_PATTERN);
		...
		if (employee.getEmployeeMotherMaidenName().length() > 30) {
			errors.rejectValue("employeeMotherMaidenName",
					"employeeMotherMaidenName.incorrect",
