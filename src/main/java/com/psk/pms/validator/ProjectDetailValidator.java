...
public class ProjectDetailValidator extends BaseValidator implements Validator {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ProjectDetailValidator.class);
	...
	public void validate(Object target, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors,
				"aliasContractorName", "required.aliasContractorName",
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "addSecurityDeposit",
				"required.addSecurityDeposit",
				"Enter Additional Security Deposit Amount.");
		...
		if (!StringUtils.isNullOrEmpty(projectDetail.getAgreementValue())) {
			...
			if (!matcher.matches()) {
				errors.rejectValue("agreementValue",
						"agreementValue.incorrect",
						"Enter a numeric value and only a single dot is allowed");
		...
		if (!StringUtils.isNullOrEmpty(projectDetail.getLessPercentage())) {
			...
			if (!matcher.matches()) {
				errors.rejectValue("lessPercentage",
						"lessPercentage.incorrect",
						"Enter a numeric value and only a single dot is allowed");
		...
		if (!StringUtils.isNullOrEmpty(projectDetail.getAddSecurityDeposit())) {
			...
			if (!matcher.matches()) {
				errors.rejectValue("addSecurityDeposit",
						"addSecurityDeposit.incorrect",
						"Enter a numeric value and only a single dot is allowed");
			} else if (projectDetail.getExAmount().length() > 15) {
				errors.rejectValue("addSecurityDeposit",
						"addSecurityDeposit.incorrect",
		...
		if (projectDetail.getAliasName().length() > 50) {
			errors.rejectValue("aliasName", "aliasName.incorrect",
					"Field must not exceed 50 characters.");
		} else if (!"Y".equalsIgnoreCase(projectDetail.getIsUpdate())) {
			boolean isAliasProjectAlreadyExisting = projectService
					.isAliasProjectAlreadyExisting(projectDetail.getAliasName());
			if (isAliasProjectAlreadyExisting) {
				errors.rejectValue("aliasName", "aliasName.incorrect",
						"Alias Project Name Already Found To Be Existing.");
			}
		}
		if (projectDetail.getAgreementNo().length() > 50) {
			errors.rejectValue("agreementNo", "agreementNo.incorrect",
					"Field must not exceed 50 characters.");
		}
		if (projectDetail.getCerNo().length() > 30) {
			errors.rejectValue("cerNo", "cerNo.incorrect",
					"Field must not exceed 30 characters");
		}
		if (projectDetail.getContractorName().length() > 50) {
			errors.rejectValue("contractorName", "contractorName.incorrect",
					"Field must not exceed 50 characters.");
		}

		if (StringUtils.isNullOrEmpty(projectDetail.getLessPercentage())) {
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "exPercentage",
					"required.exPercentage",
					"Enter one among Excess or Less in Percentage.");
		}

		if (!StringUtils.isNullOrEmpty(projectDetail.getExPercentage())
				&& !StringUtils
						.isNullOrEmpty(projectDetail.getLessPercentage())) {
			errors.rejectValue("exPercentage", "exPercentage.incorrect",
