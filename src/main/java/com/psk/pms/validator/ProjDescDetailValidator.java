...
public class ProjDescDetailValidator extends BaseValidator implements Validator {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ProjDescDetailValidator.class);
	...
	public void validate(Object target, Errors errors) {
		...
		if (!StringUtils.isNullOrEmpty(projectDescDetail.getPricePerQuantity())) {
			...
			if (!matcher.matches()) {
				errors.rejectValue("pricePerQuantity",
						"pricePerQuantity.incorrect",
						"Enter a numeric value and only a single dot is allowed");
		...
		if (!"Y".equalsIgnoreCase(projectDescDetail.getIsUpdate())) {
			LOGGER.info("Project description detail is update: "
					+ projectDescDetail.getIsUpdate());
			LOGGER.info(" project description detail Id "
					+ projectDescDetail.getProjId());
			boolean isAliasDescriptionAlreadyExisting = projectDescriptionService
					.isAliasDescriptionAlreadyExisting(projectDescDetail);
			if (isAliasDescriptionAlreadyExisting) {
				errors.rejectValue("aliasDescription",
						"aliasDescription.incorrect",
						"Alias Description Already Found To Be Existing.");
			}
		}
		...
		if (!"Y".equalsIgnoreCase(projectDescDetail.getIsUpdate())) {
			...
			if (isAliasDescriptionAlreadyExisting) {
				errors.rejectValue("serialNumber", "serialNumber.incorrect",
						"Serial Number Already Found To Be Existing.");
			}
		}
