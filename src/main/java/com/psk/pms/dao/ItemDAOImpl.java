...
public class ItemDAOImpl implements ItemDAO {
	...
	public DescItemDetail getDataDescription(final DescItemDetail descItemDetail) {
		String sql = "Select * from projdescitem where ProjDescId = "
				+ descItemDetail.getProjDescId() + " and ProjDescSerial = '"
				+ descItemDetail.getProjDescSerial() + "'";
	...
	private ProjectConfiguration.ItemDetail buildPricedItem(
	...
	public void deleteItemByProjectId(Integer projectId) {
		...
		int noOfrowsDeleted = jdbcTemplate.update(
				DELETEPROJDESCITEMBYPROJECTID, new Object[] { projectId });
	...
	public void deleteItemBySubProjectId(Integer subProjectId) {
		...
		int noOfrowsDeleted = jdbcTemplate
				.update(DELETEPROJDESCITEMBYSUBPROJECTID,
	...
	public void deleteItemByProjectDescriptionId(String projectDescId) {
		...
		int noOfrowsDeleted = jdbcTemplate.update(
				DELETEPROJDESCAITEMBYPROJECTDESCID,
				new Object[] { projectDescId });
		LOGGER.info("No of rows deleted :" + noOfrowsDeleted);
	}
	...
	public void deleteItemByProjectDescItemId(Integer projectDescItemId) {
		...
		int noOfrowsDeleted = jdbcTemplate.update(
				DELETEPROJDESCAITEMBYPROJECTDESCITEMID,
				new Object[] { projectDescItemId });
		LOGGER.info("No of rows deleted :" + noOfrowsDeleted);
	}
	...
	public List<ItemDetail> getDescItemNames(Map<String, Object> request) {
		...
		if ("" != request.get("itemName")) {
			sql = "select itemName, itemUnit, itemPrice from pricedetail where projectId = '"
					+ request.get("projectId")
					+ "' and subProjectId = '"
					+ request.get("subProjectId")
					+ "' and itemType = '"
					+ request.get("itemType")
					+ "' and itemName LIKE '%"
					+ request.get("itemName") + "%' and active = '1'";
			rows = jdbcTemplate.queryForList(sql);
		}
		for (Map<String, Object> row : rows) {
			ItemDetail itemDetail = new ItemDetail();
			...
			itemDetail.setItemName((String) row.get("itemName"));
			itemDetail.setItemUnit((String) row.get("itemUnit"));
			itemDetail.setItemPrice(((BigDecimal) row.get("itemPrice"))
					.toString());
			itemsDetail.add(itemDetail);
		}
		return itemsDetail;
	...
	public List<ItemDetail> getBaseItemNames(Map<String, Object> request) {
		...
		List<Map<String, Object>> rows = null;
		if ("" != request.get("itemName")) {
			sql = "select itemType, itemName, itemUnit, itemPrice from govestpricedetail where itemType = '"
					+ request.get("itemType")
					+ "' and itemName LIKE '%"
					+ request.get("itemName") + "%' and active = '1'";
	...
	public List<String> fetchItemTypes() {
		...
		List<String> itemTypes = jdbcTemplate.queryForList(FETCHITEMTYPES,
				String.class);
	...
	public List<String> fetchUniqueItemUnits() {
		...
		List<String> uniqueItemUnits = jdbcTemplate.queryForList(
				FETCHUNIQUEITEMUNIT, String.class);
	...
	public boolean configureItemPrice(
			final ProjectConfiguration projectItemConfiguration) {
		deactivateExistingPrices(projectItemConfiguration);
		jdbcTemplate.batchUpdate(INSERTPRICEFORITEMS,
				new BatchPreparedStatementSetter() {
					@Override
					public void setValues(PreparedStatement ps, int i)
							throws SQLException {
						ProjectConfiguration.ItemDetail itemDetail = projectItemConfiguration
								.getItemDetail().get(i);
						...
						ps.setDate(7, new java.sql.Date(Calendar.getInstance()
								.getTimeInMillis()));
					}
	...
	private void deactivateExistingPrices(
			ProjectConfiguration projectItemConfiguration) {
		jdbcTemplate.update(DEACTIVATEEXISTINGPRICES,
				projectItemConfiguration.getProjId(),
				projectItemConfiguration.getSubProjId());
	}
	...
	public ProjectConfiguration getProjectItemConfiguration(
		...
		String sql;
		if (isEditSubProject) {
			LOGGER.info("Into Edit Sub Project " + isEditSubProject
					+ " Sub Project Id" + projectConfiguration.getSubProjId());
			sql = "Select * from pricedetail where active = 1 and subProjectId = "
					+ projectConfiguration.getSubProjId();
		} else {
			sql = "Select * from pricedetail where active = 1 and projectId = "
					+ projectConfiguration.getProjId() + " and subProjectId = "
					+ projectConfiguration.getSubProjId();
		}
	...
	public List<String> getItemTypes() {
		...
		List<String> types = jdbcTemplate.queryForList(PROJECT_ITEM_TYPES,
				String.class);
		return types;
	}
	...
	public List<ProjectItemDescription> getProjectItemDescription(
		...
		String sql = "select pdi.ProjDescId,pdi.ProjDescSerial,pd.AliasDescription,\n"
				...
				+ "pdi.ProjDescId = pd.ProjDescId and pdi.ProjId = '"
				+ projId
				+ "' and pdi.itemName = '" + itemName + "'";
		List<Map<String, Object>> rows = jdbcTemplate.queryForList(sql);
		...
		for (Map<String, Object> row : rows) {
			...
			projectItemDescription.setProjectDescId((Integer) row
					.get("ProjDescId"));
			projectItemDescription.setProjectDescSerialNumber((String) row
					.get("ProjDescSerial"));
			...
			projectItemDescription.setAliasDescription((String) row
