...
public class ProjectDescriptionDAOImpl implements ProjectDescriptionDAO {
	...
	public ProjDescDetail getGovProjectDescDetail(String projDescId,String descType) {
		String	sql = projDescDetail + ",  p.AliasProjName FROM "+ DescriptionType.getdescriptionTableName(descType) +" as d " + "JOIN project as p ON d.ProjId = p.ProjId WHERE d.ProjDescId = " + projDescId;
		ProjDescDetail projDescDetail = null;
		List < Map < String, Object >> rows = jdbcTemplate.queryForList(sql);
		for (Map < String, Object > row: rows) {
			projDescDetail = buildProjectDescDetail(row);
