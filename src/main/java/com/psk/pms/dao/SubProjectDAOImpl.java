...
public class SubProjectDAOImpl implements SubProjectDAO {
    ...
    public Map<String, String> getSubAliasProjectNames(String projectId, String employeeId) {
        ...
        if (!isEmpty(employeeId) && !isEmpty(projectId)) {
            sql = "select SubProjId, AliasSubProjName from subproject where ProjId in (select projectId from authoriseproject where empId = ?) and ProjId = ? ";
            rows = jdbcTemplate.queryForList(sql, employeeId, projectId);
        } else {
            if (isEmpty(employeeId) && !isEmpty(projectId)) {
                sql = "select SubProjId, AliasSubProjName from subproject where ProjId = ?";
                rows = jdbcTemplate.queryForList(sql, projectId);
            } else {
