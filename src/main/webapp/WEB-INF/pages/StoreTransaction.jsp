...
<script>
	...
	function insertDispatchDetailRow() {
		var dispatchDetailTable = document
				.getElementById('dispatchDetailTable');
	...
	function saveDispatchedItem() {
		...
		if (1 == len - 1) {
			...
			var totalQuantity = itemTable.rows[1].cells[1]
					.getElementsByTagName('input')[0].value;
			var requestedQuantity = itemTable.rows[1].cells[2]
					.getElementsByTagName('input')[0].value;
			var obj = new DispatchItems(itemName, totalQuantity,
					requestedQuantity);
			if (itemName && totalQuantity && requestedQuantity
					|| !(itemName && totalQuantity && requestedQuantity)) {
			...
			for (i = 1; i <= len - 1; i++) {
				...
				var totalQuantity = itemTable.rows[i].cells[1]
						.getElementsByTagName('input')[0].value;
				var requestedQuantity = itemTable.rows[i].cells[2]
						.getElementsByTagName('input')[0].value;
				var obj = new DispatchItems(itemName, totalQuantity,
						requestedQuantity);
				if (itemName && totalQuantity && requestedQuantity
		...
		dispatchDetailForm["employeeId"] = document
	...
	function deleteItemRow(row) {
		...
		if (noOfRow > 2) {
			...
			document.getElementById('dispatchDetailTable').rows[1].cells[0]
					.getElementsByTagName('input')[0].value = '';
			document.getElementById('dispatchDetailTable').rows[1].cells[1]
					.getElementsByTagName('input')[0].value = '';
			document.getElementById('dispatchDetailTable').rows[1].cells[2]
					.getElementsByTagName('input')[0].value = '';
		}
	}
</script>
					...
					<table id="dispatchDetailTable" class="gridView">
						...
						<tbody>
							<tr>
								...
								<td><input class="storeDetails" name="requestedQuantity" id="requestedQuantity"
									type="text" placeholder="Enter Requested Quantity" /></td>
								<td align="center"><a class="storeDetails" id="deleteItem"
									onclick="deleteItemRow(this)"> <img
					...
					<table>
						<tr>
							...
							<td><input class="button" type="button" value="Add"
								onclick="insertDispatchDetailRow()" /></td>
							<td><input class="button" type="button" value="Submit"
								onclick="saveDispatchedItem()" /></td>
							<td></td>
						</tr>
					</table>
				</center>
