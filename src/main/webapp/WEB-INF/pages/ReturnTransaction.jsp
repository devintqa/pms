                        ...
                        $.ajax({    type: "GET",
                            ...
                            success: function (response) {
                                if (response.success) {
                                    ...
                                    if (obj != '') {
                                        document.getElementById('returnDetailTable').rows[1].remove();
                                    }
                                } else {
                                    $("#returnDetailTable");
                                    $("#returnDetailTable").hide();
                                    $("#fieldUser").prop('selectedIndex', 0);
...
function fillItems(item) {
    var row = document.getElementById('returnDetailTable').rows[1].cloneNode(true);
    var len = document.getElementById('returnDetailTable').rows.length;
    var i = 0;
    var itemName = row.cells[i].getElementsByTagName('input')[0];
    itemName.id += len;
    itemName.value = item.itemName;
    i++;
    var dispatchedQuantity = row.cells[i].getElementsByTagName('input')[0];
    dispatchedQuantity.id += len;
    dispatchedQuantity.value = item.dispatchedQuantity;
    document.getElementById('returnDetailTable').appendChild(row);
}
...
function saveReturnedItem() {
    ...
    if (1 == len - 1) {
        ...
        var dispatchedQuantity = itemTable.rows[1].cells[1].getElementsByTagName('input')[0].value;
        ...
        for (i = 1; i <= len - 1; i++) {
            ...
            var dispatchedQuantity = itemTable.rows[i].cells[1].getElementsByTagName('input')[0].value;
        ...
        <form:form method="POST" commandName="returnDetailForm"
            ...
            <center>
                ...
                <table id="returnDetailTable" class="gridView">
                    ...
                    <tbody>
                    <tr>
                        <td><input name="itemName" id="dispatchedQuantity" type="text"
                                   readonly="true"/></td>
                        <td><input name="dispatchedQuantity" id="dispatchedQuantity" type="text"
                                   readonly="true"/></td>
                        <td><input name="returnedQuantity" id="returnedQuantity" type="text"
                                   placeholder="Enter Returned Quantity"  value="0"/></td>
                    </tr>
                    </tbody>
                </table>
